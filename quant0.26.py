...
zz50 = pd.read_excel('./东财看涨看跌数据_指数分类.xlsx',sheet_name='中证50_index_data')
...
allIndex=allIndex[allIndex.index<2000]
#修改参数来改变串行股票数量
...
def ExcelFormatChange(sheet):
    ...
    for m in sheet.iter_rows(min_row=2, max_row=sheet.max_row, min_col=2, max_col=sheet.max_column):
        for n in m:
            ...
            #检查单元格内容是否为浮点数
                if n.value > 0.6:
                    n.fill = PatternFill(start_color='FF0000', end_color='FF0000', fill_type='solid')
                    #将大于0.6的单元格背景填充为红色
...
async def main():
    ...
    for i in range(len(allIndex)):
        ...
        if dfkccy50.shape[1]==51 and t50==0:
            zz50=zz50.append(dfkccy50)
            zz50.to_excel(wb, sheet_name='中证50_index_data', index=False,)
            ExcelFormatChange(wb.sheets['中证50_index_data'])
            wb.save()
            t50=t50+1
